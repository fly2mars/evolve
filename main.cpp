...
void main(int argc, char* argv[])
{
	...
	try {
		...
		{
			iteratorTimes = option.findOptionValue("-a").ConvertToInteger();
		}
	}
	catch (std::exception &e)
	{
	...
	if (dValue) nOctreeDeep = dValue;
	if (rValue) nResolution = rValue;
	if (fValue != 0) fThreshold = fValue;
	...
	optimizer.set_iter_times(iteratorTimes);
	optimizer.set_octree_level(nOctreeDeep);
	...
	int nVolume_per_dim = pow(2, nOctreeDeep);
